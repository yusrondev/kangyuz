...
@section('active-menu-user-management', 'mm-active')
@section('title-content', 'User Management')
@section('description-content', 'Data user')
@section('content')
...
<script>
    ...
    $("body").on("click", ".btn-reset-task", function() {
        console.log("hai");
        jQuery.ajax({
            processData: false,
            contentType: false,
            url: `user-management/${ $(this).data('id') }/reset-task`,
            method: 'POST',
            dataType: "JSON",
            success: function(result) {
                console.log(result);
    ...
    function refresh_data(){
        jQuery.ajax({
            processData: false,
            contentType: false,
            url: "{{ route('user-management.show', 0) }}",
            method: 'GET',
            dataType: "JSON",
            success: function(result) {
                ...
                result.forEach(function(val){
                    ...
                    forTable += `
                        <tr>
                            ...
                            <td>${val.flag.name}</td>
                            <td>${(val.level == 1) ? "admin" : "user"}</td>
                            <td>

                                <form action="destroy-flag/${val.id}" class="form-delete" method="POST">
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-info btn-sm btn-reset-task" data-id="${val.id}">
                                            <i class="pe-7s-refresh-2"></i>
                                        </button>
                                        <button type="button" class="btn btn-warning btn-sm btn-edit" data-id="${val.id}" data-name="${val.name}">
                                            <i class="fa fa-edit"></i>
                                        </button>
                                        <button class="btn btn-danger btn-sm d-none" type="submit"><i class="fa fa-trash"></i></button>
                                    </div>
                                </form>
                            </td>
                        </tr>
